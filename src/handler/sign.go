...
func (r *RequestHandler) Sign() Response {
	...
	if body.MessageType == "" {
		body.MessageType = types.MessageTypeRaw
	}

	if !(body.MessageType == types.MessageTypeRaw || body.MessageType == types.MessageTypeDigest) {
	...
	if body.MessageType == types.MessageTypeDigest {
